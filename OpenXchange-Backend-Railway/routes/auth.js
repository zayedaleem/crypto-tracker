...
// Vercel (frontend) and Railway (backend) are on different domains.
// Browsers block cookies across origins unless:
//   - sameSite is 'none'  (allows cross-site)
//   - secure is true      (required when sameSite='none')
//   - The backend is on HTTPS (Railway provides this automatically)
const IS_CROSS_ORIGIN = process.env.ALLOWED_ORIGINS &&
  !process.env.ALLOWED_ORIGINS.includes('localhost');

const COOKIE_OPTS = {
  httpOnly: true,
  secure:   true,                          // Always true — Railway is always HTTPS
  sameSite: IS_CROSS_ORIGIN ? 'none' : 'lax', // 'none' for Vercel↔Railway cross-domain
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (user.force_2fa_setup || !user.totp_enabled) {
        ...
        return res.json({ require2faSetup: true, setupToken }); // token in body for cross-domain fallback
      }
      ...
      res.json({
        success: true,
        token,   // also returned in body for cross-domain Authorization header fallback
        user: {
          id:       user.id,
...
router.get('/setup-2fa', async (req, res) => {
  // Accept cookie, setup_token cookie, or Authorization header (for cross-domain fallback)
...
router.post('/verify-2fa',
  ...
  async (req, res) => {
    ...
    res.json({
      success: true,
      token: fullToken, // also in body for cross-domain Authorization header fallback
